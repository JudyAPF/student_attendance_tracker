    ...
    <div class="col-lg-12">
        ...
        <div class="card mb-4">
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold" style="color: #0C24DB;">Create Class</h6>
            </div>
            <div class="card-body">
                <form method="post" action="
                @if(isset($class))
                    {{ route('admin.update_class', $class->id) }}
                @else
                    {{ route('admin.create_class') }}
                @endif">
                    @csrf
                    <div class="form-group row mb-3">
                        <div class="col-xl-6">
                            <label class="form-control-label">Class Name<span class="text-danger ml-2">*</span></label>
                            <input type="text" class="form-control" name="className" id="className" value="{{ isset($class) ? $class->className : '' }}" placeholder="Class Name">
                        </div>
                    </div>
...
<div class="row mb-3">
    <div class="col-lg-12">
        ...
        <div class="card mb-4">
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold" style="color: #0C24DB;">All Classes</h6>
            </div>
                            ...
                            <table class="table align-items-center table-flush table-hover dataTable no-footer" id="dataTableHover" role="grid" aria-describedby="dataTableHover_info">
                                ...
                                <tbody>
                                    ...
                                    <tr role="row" class="odd">
                                        <td class="sorting_1">{{ $class->id }}</td>
                                        <td>{{ $class->className }}</td>
                                        <td>
                                            <a href="{{ route('admin.edit_class', $class->id) }}" class="btn"style="background-color: #0C24DB; color:#fff;">Edit</a>
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.delete_class', $class->id) }}" class="btn btn-danger">Delete</a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
